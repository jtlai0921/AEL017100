...
Public Sub AddLinePoint (LD As LineData, X As String, Y As Float, ShowTick As Boolean)
...
Public Sub AddLineMultiplePoints(LD As LineData, X As String, YArray() As Float, ShowTick As Boolean)
	If LD.Points.IsInitialized = False Then LD.Points.Initialize
	Dim p As LinePoint
	p.Initialize
	p.X = X
...
Public Sub AddLineColor(LD As LineData, Color As Int)
	If LD.LinesColors.IsInitialized = False Then LD.LinesColors.Initialize
	LD.LinesColors.Add(Color)
End Sub

Public Sub DrawLineChart(G As Graph, LD As LineData, BackColor As Int)
	If LD.Points.Size = 0 Then
		Log("Missing line points.")
		Return
	End If
...
Private Sub calcPointToPixel(py As Float, G As Graph) As Int
...
Private Sub drawGraph (G As Graph, Canvas As Canvas, Target As View, Points As List, Bars As Boolean, BarsWidth As Int)
	...
	For i = 0 To (G.YEnd - G.YStart) / G.Yinterval + 1
		...
		End If
		Canvas.DrawText(NumberFormat(yValue, 1, 2), GI.originX - 8dip, y + 5dip,Font.CreateNew(12), G.AxisColor, "RIGHT")
	Next
	'Draw titles
	Canvas.DrawText(G.Title, Target.Width / 2, 30dip, Font.CreateNew(15), G.AxisColor, "CENTER")
	Canvas.DrawText(G.XAxis, Target.Width / 2, GI.originY + 45dip, Font.CreateNew(14), G.AxisColor, "CENTER")
	Canvas.DrawTextRotated(G.YAxis, 20dip, Target.Height / 2, Font.CreateNew(14), G.AxisColor, "CENTER", -90)
	'Draw X axis
	...
	For i = 0 To Points.Size - 1
		...
		If p.ShowTick Then
			...
			If p.x.Length > 0 Then
				Canvas.DrawTextRotated(p.x, x, GI.originY + 12dip, Font.CreateNew(12), G.AxisColor, "RIGHT", -45)
			End If
		End If
	Next
End Sub
...
Public Sub AddBarPoint (BD As BarData, X As String, YArray() As Float)
...
Public Sub AddBarColor(BD As BarData, Color As Int)
	If BD.BarsColors.IsInitialized = False Then BD.BarsColors.Initialize
	BD.BarsColors.Add(Color)
End Sub

Public Sub DrawBarsChart(G As Graph, BD As BarData, BackColor As Int)
	If BD.Points.Size = 0 Then
		Log("Missing bars points.")
		Return
	End If
...
Public Sub AddPieItem(PD As PieData, Name As String, Value As Float, Color As Int)
